import type { Article } from "@/engine/types";

export const articles: Article[] = [
  {
    slug: "como-elegir-tu-primera-pala-de-padel",
    title: "Cómo elegir tu primera pala de pádel",
    h1: "Cómo elegir tu primera pala de pádel: guía para principiantes",
    excerpt:
      "Elegir la primera pala genera muchas dudas: forma, balance, peso, dureza... En esta guía explicamos qué mirar realmente cuando empiezas a jugar y qué errores evitar.",
    category: "Guías",
    coverImage: "/images/articles/como-elegir-tu-primera-pala.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-03-01",
    updatedAt: "2026-06-05",
    readingMinutes: 6,
    sections: [
      {
        heading: "No compres la pala más cara que puedas permitirte",
        content:
          "Es el error más habitual: pensar que una pala de gama alta va a compensar la falta de técnica. Las palas de potencia de balance alto son, precisamente, las más exigentes de manejar. Como principiante vas a disfrutar más y progresar más rápido con una pala de balance bajo o medio, ligera y de tacto blando.",
      },
      {
        heading: "Forma redonda: la mejor aliada al empezar",
        content:
          "Las palas de forma redonda concentran el peso cerca del mango, lo que amplía el punto dulce y perdona mucho los golpes descentrados, algo inevitable cuando se está aprendiendo la técnica básica.",
      },
      {
        heading: "Cuidado con el peso y la dureza si notas molestias",
        content:
          "Si notas algo de sensibilidad en el codo o la muñeca, prioriza palas ligeras (340-350g) con núcleo de goma blanda. El sobreesfuerzo con una pala dura y pesada es una causa común de la temida epicondilitis del pádel.",
      },
      {
        heading: "No hace falta gastar mucho para empezar bien",
        content:
          "Hay palas de iniciación muy competentes por debajo de 100€. Nuestra recomendación es empezar con una pala de este rango y, pasados unos meses, dar el salto a una pala de gama media cuando ya tengas claro tu estilo de juego.",
      },
    ],
    faqs: [
      {
        question: "¿Cuánto dura una pala de pádel?",
        answer: "Con un uso de 2-3 veces por semana, una pala suele mantener sus prestaciones entre 1 y 2 años antes de empezar a perder tacto.",
      },
      {
        question: "¿Necesito una pala distinta si soy mujer?",
        answer: "No es imprescindible, pero muchas jugadoras prefieren palas más ligeras; puedes filtrar por ese criterio en nuestro selector de pala.",
      },
    ],
    relatedSlugs: [
      "mejores-palas-principiantes",
      "mejores-palas-menos-de-100",
      "epicondilitis-padel-como-elegir-material",
    ],
  },
  {
    slug: "pala-control-vs-potencia-cual-elegir",
    title: "Pala de control vs pala de potencia: cuál elegir",
    h1: "Pala de control vs pala de potencia: cuál elegir según tu juego",
    excerpt:
      "Es la duda más habitual al comprar una pala nueva. Repasamos qué diferencia realmente a una pala de control de una de potencia, y qué modelos recomendamos en cada caso.",
    category: "Guías",
    coverImage: "/images/articles/pala-control-vs-potencia.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-04-02",
    updatedAt: "2026-06-10",
    readingMinutes: 7,
    sections: [
      {
        heading: "Qué diferencia realmente a una pala de control de una de potencia",
        content:
          "La diferencia no está tanto en la marca como en tres parámetros: forma, balance y dureza del núcleo. Las palas de control suelen ser redondas, con balance bajo (el peso concentrado cerca del mango) y un núcleo más blando. Las de potencia son diamante, con balance alto (peso hacia la cabeza) y un núcleo más duro. Ese balance alto es lo que multiplica la fuerza del golpe, pero también lo que exige más técnica y físico para manejarla bien durante todo un partido.",
      },
      {
        heading: "Pala de control: para quién es y qué modelos destacan",
        content:
          "Si tu prioridad es la precisión, la construcción del punto y cuidar el brazo, una pala de control es la opción más razonable. Suelen perdonar más los golpes descentrados y son más manejables en la volea y la defensa. Dos referencias claras dentro de esta categoría:",
        productSlugs: ["babolat-technical-veron", "nox-ml10-pro-cup"],
      },
      {
        heading: "Pala de potencia: para quién es y qué modelos destacan",
        content:
          "Si ya tienes una técnica consolidada y tu estilo es agresivo, una pala de potencia te va a dar una pegada que las de control no pueden ofrecer. Eso sí: exigen más nivel físico y no son la mejor opción si notas molestias en el codo. Estas son las dos palas de potencia con mejor valoración de nuestro catálogo:",
        productSlugs: ["nox-at10-genius-18k", "bullpadel-vertex-04"],
      },
      {
        heading: "Y si no lo tienes claro: las híbridas equilibradas",
        content:
          "No todo es blanco o negro. Existe un tercer grupo de palas de forma lágrima y balance medio que reparten mejor entre control y potencia, pensadas para quien todavía está definiendo su estilo de juego o simplemente no quiere renunciar del todo a ninguna de las dos cosas.",
        productSlugs: ["head-speed-motion", "drop-shot-conqueror-5-0"],
      },
    ],
    faqs: [
      {
        question: "¿Una pala de control también sirve para rematar?",
        answer:
          "Sí, aunque con menos contundencia que una de potencia. Una pala de control bien construida permite rematar con solvencia, solo que no vas a notar la misma pegada en los golpes de ataque.",
      },
      {
        question: "¿Puedo pasar directamente de una pala de iniciación a una de potencia?",
        answer:
          "No es lo más recomendable. Lo habitual es pasar primero por una pala equilibrada o de control de gama media antes de dar el salto a una de potencia pura, para no perder precisión de golpeo.",
      },
    ],
    relatedSlugs: [
      "mejores-palas-control",
      "mejores-palas-potencia",
      "selector-pala-control",
      "selector-pala-potencia",
    ],
  },
  {
    slug: "zapatillas-padel-segun-tipo-pisada",
    title: "Qué zapatillas de pádel elegir según tu tipo de pisada",
    h1: "Qué zapatillas de pádel elegir según tu tipo de pisada",
    excerpt:
      "La pisada influye más que la marca a la hora de elegir zapatillas de pádel. Te explicamos cómo identificar si eres pronador o neutro y qué modelo encaja con cada perfil.",
    category: "Guías",
    coverImage: "/images/articles/zapatillas-segun-pisada.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-04-08",
    updatedAt: "2026-06-10",
    readingMinutes: 5,
    sections: [
      {
        heading: "Por qué la pisada importa más que la marca",
        content:
          "El pádel obliga a frenadas y cambios de dirección constantes, mucho más bruscos que correr en línea recta. Una zapatilla pensada para tu tipo de pisada reduce el riesgo de sobrecargas y torceduras de tobillo, algo que ninguna marca por sí sola puede compensar si el modelo no encaja con cómo apoyas el pie.",
      },
      {
        heading: "Pisada pronadora: qué buscar",
        content:
          "Si notas que el pie tiende a girar hacia dentro al apoyar (pronación), prioriza zapatillas con refuerzos laterales y buena estabilidad en la suela, para compensar ese giro en los cambios de dirección rápidos.",
        productSlugs: ["nox-ml10-hexa"],
      },
      {
        heading: "Pisada neutra: la más versátil",
        content:
          "Con pisada neutra tienes más margen: puedes priorizar amortiguación y polivalencia sin preocuparte tanto por la estabilidad lateral. Es el perfil que mejor se adapta a zapatillas generalistas pensadas para pistas rápidas.",
        productSlugs: ["asics-gel-padel-pro-5"],
      },
      {
        heading: "Cómo saber qué pisada tienes sin ir al podólogo",
        content:
          "Un truco casero: moja la planta del pie y pisa sobre una superficie que deje marca (cartón, folio). Si la huella se ve casi completa y ancha, tiendes a la pronación; si se ve una franja fina en el centro, es más probable que tengas pisada neutra o incluso supinadora. No sustituye a un estudio de la pisada real, pero da una primera pista.",
      },
    ],
    faqs: [
      {
        question: "¿Puedo jugar al pádel con zapatillas de running?",
        answer:
          "No es lo recomendable: las zapatillas de running están pensadas para desplazamiento lineal, no para los frenazos y cambios de dirección del pádel, y su suela no ofrece el agarre necesario en pista.",
      },
    ],
    relatedSlugs: ["selector-zapatillas-pronador", "selector-zapatillas-amortiguacion"],
  },
  {
    slug: "epicondilitis-padel-como-elegir-material",
    title: "Epicondilitis y pádel: cómo elegir material que cuide tu codo",
    h1: "Epicondilitis y pádel: cómo elegir material que cuide tu codo",
    excerpt:
      "El codo de tenista es una de las lesiones más frecuentes entre jugadores de pádel amateur. La pala y el overgrip que uses pueden marcar la diferencia. Te contamos qué mirar.",
    category: "Guías",
    coverImage: "/images/articles/epicondilitis-material-padel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-04-15",
    updatedAt: "2026-06-10",
    readingMinutes: 6,
    sections: [
      {
        heading: "Qué es la epicondilitis y por qué es tan común en el pádel",
        content:
          "La epicondilitis, conocida como codo de tenista, es la inflamación de los tendones que unen el antebrazo con la parte externa del codo. En el pádel aparece sobre todo por la vibración repetida que transmite la pala en cada golpe, agravada si el material no es el adecuado para tu nivel o tu técnica todavía no está consolidada.",
      },
      {
        heading: "La pala es la primera decisión: balance bajo y tacto blando",
        content:
          "Un balance bajo (peso cerca del mango) y un núcleo de goma blanda reducen la vibración que llega al brazo en cada impacto. Estas son las palas de nuestro catálogo que priorizamos para jugadores con molestias en el codo:",
        productSlugs: ["babolat-technical-veron", "nox-ml10-pro-cup", "siux-adrenaline-ibai-edition"],
      },
      {
        heading: "El overgrip también influye en la vibración que llega al brazo",
        content:
          "Un overgrip en mal estado o demasiado fino obliga a apretar más el mango para mantener el agarre, lo que a su vez tensa más el antebrazo. Un overgrip con buen tacto y renovado con regularidad ayuda más de lo que parece.",
        productSlugs: ["babolat-vs-original"],
      },
      {
        heading: "Otros hábitos que ayudan a proteger el codo",
        content:
          "Además del material, ayuda calentar bien el antebrazo antes de jugar, evitar golpear con el brazo completamente estirado y no forzar partidos si ya notas molestias. Si el dolor persiste más de un par de semanas, lo razonable es consultar con un fisioterapeuta antes de que se cronifique.",
      },
    ],
    faqs: [
      {
        question: "¿Una pala más cara duele menos el codo?",
        answer:
          "No necesariamente: lo que reduce la vibración es el balance bajo y el núcleo blando, no el precio. Hay palas de gama media muy recomendables para cuidar el codo, como las que mencionamos en este artículo.",
      },
      {
        question: "¿Cambiar de pala cura la epicondilitis?",
        answer:
          "No: una pala más blanda puede ayudar a no agravar las molestias y a jugar más cómodo, pero ante una lesión activa lo recomendable es acudir a un fisioterapeuta o médico deportivo.",
      },
    ],
    relatedSlugs: [
      "mejores-palas-epicondilitis",
      "selector-pala-epicondilitis",
      "como-elegir-tu-primera-pala-de-padel",
    ],
  },
  {
    slug: "cuanto-debe-pesar-tu-pala-de-padel",
    title: "Cuánto debe pesar tu pala de pádel según tu nivel",
    h1: "Cuánto debe pesar tu pala de pádel según tu nivel y tu físico",
    excerpt:
      "El peso de la pala afecta tanto a la potencia como a la manejabilidad y a la fatiga del brazo. Te explicamos qué rango de peso encaja con cada nivel de juego.",
    category: "Guías",
    coverImage: "/images/articles/peso-pala-segun-nivel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-04-22",
    updatedAt: "2026-06-10",
    readingMinutes: 6,
    sections: [
      {
        heading: "Por qué el peso importa tanto como la forma",
        content:
          "A más peso, más potencia de salida en el golpeo, pero también más esfuerzo para mover la pala en la volea y más fatiga acumulada durante el partido. No existe un peso «correcto» universal: depende de tu nivel físico, tu estilo de juego y de si arrastras o no molestias articulares.",
      },
      {
        heading: "Palas ligeras (340-350g): a quién van bien",
        content:
          "Recomendables para quien empieza, para perfiles defensivos y para jugadores que priorizan la manejabilidad en la red por encima de la pegada. También es el rango más seguro si tienes molestias en el codo o la muñeca.",
        productSlugs: ["siux-adrenaline-ibai-edition", "wilson-bela-team"],
      },
      {
        heading: "Palas medias (350-360g): el punto intermedio",
        content:
          "El rango más habitual para nivel intermedio: suficiente potencia para progresar sin sacrificar demasiado el manejo. Es donde recomendamos empezar a mirar si ya tienes la técnica consolidada pero no quieres irte a los extremos.",
        productSlugs: ["head-speed-motion", "oxdog-ultimate-pro-light-2024"],
      },
      {
        heading: "Palas pesadas (360g o más): solo para nivel avanzado",
        content:
          "Aquí están las palas de potencia pura, pensadas para jugadores con técnica consolidada y buen nivel físico. Con este peso la pala perdona menos los golpes mal ejecutados y exige más del brazo partido tras partido.",
        productSlugs: ["nox-at10-genius-18k", "bullpadel-vertex-04"],
      },
    ],
    faqs: [
      {
        question: "¿10 gramos de diferencia se notan tanto?",
        answer:
          "Sí: en una pala de pádel, 10-15 gramos de diferencia son perceptibles en la maniobrabilidad, sobre todo en la volea y en los cambios rápidos de posición de la pala.",
      },
      {
        question: "¿Puedo pedir que me añadan peso a una pala ligera?",
        answer:
          "Existen sistemas de plomos adhesivos para ajustar el peso y el balance de una pala ya comprada, aunque lo más recomendable es partir de un modelo que ya se ajuste a lo que buscas.",
      },
    ],
    relatedSlugs: ["como-elegir-tu-primera-pala-de-padel", "mejores-palas-control"],
  },
  {
    slug: "overgrips-sudoracion-manos-padel",
    title: "Por qué se te escapa la pala de las manos (y cómo evitarlo)",
    h1: "Por qué se te escapa la pala de las manos y cómo evitarlo con el overgrip adecuado",
    excerpt:
      "El overgrip es el accesorio más barato de todo el equipamiento y, sin embargo, el que más influye en si sientes o no que la pala se te escapa de las manos. Te contamos cómo elegirlo.",
    category: "Guías",
    coverImage: "/images/articles/overgrips-sudoracion-padel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-04-29",
    updatedAt: "2026-06-10",
    readingMinutes: 4,
    sections: [
      {
        heading: "El accesorio más barato con más impacto en tu juego",
        content:
          "Un overgrip cuesta unos pocos euros y se cambia en cinco minutos, pero un overgrip gastado o inadecuado para tu nivel de sudoración puede arruinar tu sensación de agarre en pleno partido, por muy buena que sea la pala que lleves debajo.",
      },
      {
        heading: "Manos muy sudorosas: overgrip perforado",
        content:
          "Si notas que sudas mucho de manos, un overgrip con superficie perforada evacúa mejor la humedad y mantiene el agarre estable durante todo el partido, aunque el tacto sea algo más rugoso que uno liso.",
        productSlugs: ["siux-pro-perforated-x60"],
      },
      {
        heading: "Manos normales: prioriza el tacto",
        content:
          "Si no sudas en exceso, un overgrip liso te va a dar más sensibilidad y precisión en el golpeo, sin la absorción extra que necesitarías con manos más sudorosas.",
        productSlugs: ["babolat-vs-original"],
      },
      {
        heading: "Cada cuánto hay que cambiar el overgrip",
        content:
          "Como referencia, si juegas 2-3 veces por semana, un overgrip suele aguantar entre 2 y 4 semanas antes de perder su capacidad de absorción. Si notas que se desliza o que ya no absorbe como al principio, es el momento de cambiarlo.",
      },
    ],
    faqs: [
      {
        question: "¿Puedo poner un overgrip encima de otro?",
        answer:
          "Sí, es habitual poner un overgrip nuevo sobre uno antiguo para ganar grosor, aunque si el de debajo ya está muy degradado es mejor quitarlo antes para no perder sensibilidad.",
      },
    ],
    relatedSlugs: ["selector-overgrip-sudor"],
  },
  {
    slug: "forma-diamante-vs-lagrima-vs-redonda-padel",
    title: "Forma diamante, lágrima o redonda: qué diferencia hay realmente",
    h1: "Forma diamante, lágrima o redonda: qué diferencia hay entre las palas de pádel",
    excerpt:
      "La forma es, junto al balance, lo que más condiciona el carácter de una pala. Explicamos qué aporta cada forma y cuál encaja mejor con cada estilo de juego.",
    category: "Guías",
    coverImage: "/images/articles/forma-pala-padel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-07-01",
    updatedAt: "2026-07-01",
    readingMinutes: 5,
    sections: [
      {
        heading: "Redonda: el punto dulce más amplio",
        content:
          "En una pala redonda, el peso se concentra cerca del mango, lo que sitúa el punto dulce (la zona de máximo rendimiento al golpear) más cerca del centro geométrico de la pala. Es la forma que más perdona los golpes descentrados, ideal para quien empieza o prioriza el control.",
      },
      {
        heading: "Lágrima: el término medio",
        content:
          "La forma lágrima desplaza algo de peso hacia la cabeza sin llegar al extremo de la diamante, buscando un equilibrio entre punto dulce amplio y algo más de pegada. Es la forma más habitual en palas de gama media pensadas para nivel intermedio.",
        productSlugs: ["lok-rebel-pro", "head-extreme-pro"],
      },
      {
        heading: "Diamante: máxima potencia, punto dulce reducido",
        content:
          "En la forma diamante el peso se concentra en la parte superior de la cabeza, lo que multiplica la potencia en el golpeo pero reduce el margen de error: los golpes descentrados se notan mucho más que en una redonda. Exige buena técnica y suele ir asociada a niveles avanzados.",
        productSlugs: ["black-crown-piton-5-0", "adidas-metalbone-3-3"],
      },
      {
        heading: "Entonces, ¿qué forma elijo?",
        content:
          "Como regla general: si estás empezando o priorizas cuidar el brazo, redonda. Si ya tienes cierto nivel y quieres un equilibrio entre control y pegada, lágrima. Si tu técnica está consolidada y buscas máxima potencia, diamante.",
      },
    ],
    faqs: [
      {
        question: "¿La forma influye en el riesgo de lesión?",
        answer:
          "Indirectamente sí: las formas diamante, al concentrar más peso en la cabeza, suelen transmitir algo más de vibración en golpes descentrados, lo que puede agravar molestias en codo o muñeca si ya existen.",
      },
      {
        question: "¿Puedo notar la diferencia de forma sin fijarme en la ficha técnica?",
        answer:
          "Sí, con la vista: la redonda tiene un contorno prácticamente circular, la lágrima tiene forma de gota alargada, y la diamante tiene un vértice claramente marcado hacia la parte superior.",
      },
    ],
    relatedSlugs: ["pala-control-vs-potencia-cual-elegir", "mejores-palas-control", "mejores-palas-potencia"],
  },
  {
    slug: "cuando-cambiar-de-pala-de-padel",
    title: "Cuándo cambiar de pala de pádel: señales que no deberías ignorar",
    h1: "Cuándo cambiar de pala de pádel",
    excerpt:
      "Una pala no dura para siempre, aunque no se rompa. Repasamos las señales que indican que tu pala ha perdido prestaciones y es momento de renovarla.",
    category: "Guías",
    coverImage: "/images/articles/cuando-cambiar-pala-padel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-07-02",
    updatedAt: "2026-07-02",
    readingMinutes: 5,
    sections: [
      {
        heading: "Una pala pierde prestaciones antes de romperse",
        content:
          "El núcleo de goma o EVA de una pala se va endureciendo con el uso y los golpes repetidos, incluso sin que aparezca ninguna grieta visible. Esa pérdida de elasticidad se traduce en menos punto dulce y menos sensación de control, aunque la pala siga pareciendo intacta por fuera.",
      },
      {
        heading: "Señales claras de que toca cambiar",
        content:
          "Grietas o marcas blancas en el marco, un sonido más sordo al golpear comparado con cuando era nueva, o la sensación de que la bola 'no sale' igual que antes son las señales más habituales. También cuenta el desgaste del grip original y del protector, aunque esos son más baratos de sustituir por separado.",
      },
      {
        heading: "Con qué frecuencia hay que renovar la pala",
        content:
          "Como referencia orientativa: jugando 2-3 veces por semana, entre 12 y 24 meses suele ser el ciclo de vida útil antes de notar una pérdida de prestaciones significativa. Jugadores de competición que entrenan a diario pueden notar el desgaste en pocos meses.",
      },
      {
        heading: "Aprovecha para revisar tu nivel actual",
        content:
          "Cambiar de pala es un buen momento para reevaluar qué buscas: si tu técnica ha mejorado desde tu última compra, quizás ya puedas dar el salto a una pala más exigente en potencia sin sacrificar demasiado control.",
      },
    ],
    faqs: [
      {
        question: "¿Merece la pena reparar una pala agrietada en vez de cambiarla?",
        answer:
          "Para grietas superficiales existen resinas de reparación, pero si la grieta afecta al marco estructural, es más seguro y rentable a medio plazo cambiar de pala antes de que se rompa por completo durante un partido.",
      },
      {
        question: "¿El protector y el overgrip cuentan como parte del ciclo de vida de la pala?",
        answer:
          "No directamente: son consumibles que se cambian de forma independiente y mucho más a menudo (semanas o pocos meses) que la propia pala.",
      },
    ],
    relatedSlugs: ["como-elegir-tu-primera-pala-de-padel", "mejores-palas-avanzadas"],
  },
  {
    slug: "epicondilitis-codo-de-tenista-en-padel-como-evitarlo",
    title: "Cómo evitar el codo de tenista (epicondilitis) jugando al pádel",
    h1: "Cómo evitar el codo de tenista en pádel",
    excerpt:
      "La epicondilitis es una de las lesiones más frecuentes entre jugadores amateur de pádel. Explicamos qué la provoca y cómo el material puede ayudar a prevenirla.",
    category: "Guías",
    coverImage: "/images/articles/epicondilitis-padel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-07-03",
    updatedAt: "2026-07-03",
    readingMinutes: 6,
    sections: [
      {
        heading: "Qué es la epicondilitis y por qué es tan común en pádel",
        content:
          "La epicondilitis, conocida como codo de tenista, es una inflamación de los tendones del antebrazo que se insertan en el codo, provocada por la repetición de impactos y microvibraciones. En pádel es especialmente frecuente por la superficie rígida de la pala, muy distinta a una raqueta encordada que amortigua parte del impacto.",
      },
      {
        heading: "El material influye, y mucho",
        content:
          "Una pala dura, pesada y con balance alto transmite más vibración al brazo en cada golpeo. Cambiar a una pala más ligera, de núcleo blando y balance bajo o medio puede reducir notablemente esa transmisión, aunque no sustituye una buena técnica.",
        productSlugs: ["babolat-air-viper"],
      },
      {
        heading: "El overgrip y el encordado también cuentan",
        content:
          "Un overgrip en mal estado obliga a apretar más fuerte el mango para no perder agarre, lo que aumenta la tensión en el antebrazo. Mantenerlo en buen estado es una medida sencilla y barata de prevención.",
      },
      {
        heading: "Más allá del material: técnica y descanso",
        content:
          "Ningún cambio de material sustituye una técnica correcta ni el descanso necesario tras sobrecargar la zona. Si ya tienes molestias persistentes, lo recomendable es consultar con un fisioterapeuta antes de que la inflamación vaya a más.",
      },
    ],
    faqs: [
      {
        question: "¿Qué tipo de pala se recomienda si ya tengo epicondilitis?",
        answer:
          "Palas ligeras (340-360g), de forma redonda, balance bajo y núcleo de goma blanda. Puedes filtrar directamente por esta lesión en nuestro selector de pala.",
      },
      {
        question: "¿Una muñequera ayuda a prevenir la epicondilitis?",
        answer:
          "Una muñequera no actúa sobre el codo directamente, pero una coderas o bandas de compresión específicas sí pueden aliviar la tensión del tendón durante el juego, siempre como complemento y no como solución única.",
      },
    ],
    relatedSlugs: ["epicondilitis-padel-como-elegir-material", "mejores-palas-epicondilitis", "selector-pala-epicondilitis"],
  },
  {
    slug: "cuidados-basicos-zapatillas-padel",
    title: "Cómo cuidar tus zapatillas de pádel para que duren más",
    h1: "Cómo cuidar tus zapatillas de pádel para que duren más",
    excerpt:
      "Una zapatilla de pádel es una inversión que se puede alargar bastante con unos cuidados básicos. Repasamos los errores más comunes y cómo evitarlos.",
    category: "Guías",
    coverImage: "/images/articles/cuidado-zapatillas-padel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-07-04",
    updatedAt: "2026-07-04",
    readingMinutes: 4,
    sections: [
      {
        heading: "Usa las zapatillas solo para pádel",
        content:
          "Es tentador usar las mismas zapatillas para el día a día, pero eso acelera muchísimo el desgaste de la suela, pensada específicamente para el agarre en pista de césped artificial o cemento poroso, no para caminar por asfalto.",
      },
      {
        heading: "Dos pares en rotación, si juegas mucho",
        content:
          "Si entrenas o compites varias veces por semana, alternar entre dos pares permite que la espuma de la mediasuela recupere su forma entre sesión y sesión, alargando la vida útil de amortiguación de ambos pares.",
      },
      {
        heading: "Sécalas bien y evita el calor directo",
        content:
          "Tras jugar con calor o lluvia, deja que se sequen a temperatura ambiente, lejos de radiadores o del sol directo, que pueden deformar los materiales adhesivos de la suela.",
      },
      {
        heading: "Vigila el desgaste de la suela como indicador de pisada",
        content:
          "El patrón de desgaste te dice mucho: si se desgasta más por el borde interior, es señal de pisada pronadora, información útil para elegir tu siguiente par.",
        productSlugs: ["bullpadel-vertex-23"],
      },
    ],
    faqs: [
      {
        question: "¿Se pueden lavar en lavadora?",
        answer:
          "No se recomienda: el tambor y el centrifugado pueden dañar la estructura y los adhesivos de la zapatilla. Lo mejor es limpiarlas a mano con un cepillo suave y agua tibia.",
      },
    ],
    relatedSlugs: ["zapatillas-padel-segun-tipo-pisada", "mejores-zapatillas", "mejores-zapatillas-pronador"],
  },
  {
    slug: "guia-tallas-ropa-tecnica-padel",
    title: "Guía de tallas para ropa técnica de pádel",
    h1: "Guía de tallas para ropa técnica de pádel",
    excerpt:
      "Comprar ropa técnica de pádel por internet genera dudas de talla frecuentes. Te damos algunas claves para acertar a la primera.",
    category: "Guías",
    coverImage: "/images/articles/guia-tallas-ropa-padel.jpg",
    author: "Redacción Voleador",
    publishedAt: "2026-07-05",
    updatedAt: "2026-07-05",
    readingMinutes: 4,
    sections: [
      {
        heading: "La ropa técnica suele tallar más ajustada",
        content:
          "A diferencia de una camiseta de algodón normal, la ropa técnica de pádel está pensada para un ajuste ceñido al cuerpo que facilite el movimiento sin holguras, así que muchas marcas recomiendan pedir una talla por encima de la habitual.",
        productSlugs: ["bullpadel-camiseta-tech"],
      },
      {
        heading: "Diferencias entre marcas",
        content:
          "No todas las marcas tallan igual: algunas mantienen un patronaje más europeo estándar, mientras que otras, especialmente las de origen americano, tienden a tallar algo más grande. Revisar la tabla de medidas específica de cada producto es más fiable que guiarte solo por tu talla habitual.",
      },
      {
        heading: "Faldas y pantalones: el short interior importa",
        content:
          "En el caso de las faldas técnicas con short interior integrado, el ajuste del short suele ser más determinante que el de la propia falda a la hora de elegir talla, ya que es la parte que va más pegada al cuerpo.",
        productSlugs: ["adidas-club-skirt-falda-mujer", "wilson-falda-team"],
      },
    ],
    faqs: [
      {
        question: "¿Qué hago si estoy entre dos tallas?",
        answer:
          "Como norma general, si vas a usar la prenda para jugar (movimiento constante), es preferible pedir la talla superior antes que una prenda demasiado ajustada que limite el movimiento.",
      },
    ],
    relatedSlugs: ["cuidados-basicos-zapatillas-padel"],
  },
];
